...
// TODO Add filter!
private[scalamodules] class ServicesWatcher[I <: AnyRef](interface: Class[I])(context: BundleContext) {
  ...
  def andHandle(f: PartialFunction[ServiceEvent[I], Unit]) {
    ...
    val tracker = new ServiceTracker(context, interface.getName, null) {
